...
function main() {
    ...
    const images = Array.from(document.querySelectorAll("img[data-low][data-high]"));
    images.forEach((img, index) => {
        const lowSrc = img.dataset.low;
        const highSrc = img.dataset.high;
        if (!lowSrc || !highSrc) {
            console.warn("[demo] image missing data-low or data-high attributes, skipping", img);
            return;
        }
    ...
    window.__resourfsceLoader = loader;
